...
class RandomData:
    ...
    @classmethod
    def dense_mtx(
        cls, row: int = None, col: int = None, n_min: int = 0, n_max: int = 5000
    ):
        """generates a random (row x col) dense matrix"""
        return np.random.rand(
            row or random.randint(n_min, n_max), col or random.randint(n_min, n_max)
        )
    ...
    def list(cls, fn: Callable, n_min: int = 0, n_max: int = 5000, **kwargs) -> list:
        ...
        return [fn(**kwargs) for _ in range(random.randint(n_min, n_max))]
